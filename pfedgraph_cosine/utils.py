...
def update_graph_matrix_neighbor(cfg, graph_matrix, nets_this_round, initial_global_parameters, dw, fed_avg_freqs, lambda_1, similarity_matric, local_losses):
    ...
    graph_matrix = optimizing_graph_matrix_neighbor(cfg, graph_matrix, index_clientid, model_difference_matrix, lambda_1, fed_avg_freqs, local_losses)
    # print(f'Model difference: {model_difference_matrix[0]}')
    # print(f'Graph matrix: {graph_matrix}')
    return graph_matrix


def optimizing_graph_matrix_neighbor(cfg, graph_matrix, index_clientid, model_difference_matrix, lamba, fed_avg_freqs, local_losses):
    ...
    for i in range(model_difference_matrix.shape[0]):
        ...
        # if cfg["consider_data_quantity"] == 1:
        #     prob = cp.Problem(cp.Minimize(cp.quad_form(x, P) + q.T @ x),
        #               [G @ x <= h,
        #                A @ x == b]
        #               )
        # elif cfg["consider_data_quantity"] == 0:
...
def aggregation_by_graph(cfg, graph_matrix, nets_this_round, global_w):
    ...
    elif cfg['weighted_initial'] == 0:
        # just remain the local models from the last round
        pass
    else:
        raise ValueError("weighted_intial must be 0 or 1")
