...
def local_train_pfedgraph(args, round, nets_this_round, cluster_models, train_local_dls, val_local_dls, test_dl, data_distributions, best_val_acc_list, best_test_acc_list, benign_client_list, generalized_test_acc_list):
    
    '''
    what are nets_this_round: reference to a dict of local models, intialized with aggregated weights
    what are cluster_models: reference to a dict of personalized aggregated models
    '''
...
n_party_per_round = int(args.n_parties * args.sample_fraction) # by default: sample_fraction = 1.0
...
benign_client_list = random.sample(party_list, int(args.n_parties * (1-args.attack_ratio))) # by default: attack_ratio = 0.0
...
for i in range(cfg['client_num']):
    ...
    best_val_acc_list.append(0) # initialize with 0
...
for round in range(cfg["comm_round"]):
    ...
    mean_personalized_acc, mean_generalized_acc = local_train_pfedgraph(args, round, nets_this_round, cluster_models, train_local_dls, val_local_dls, test_dl, data_distributions, best_val_acc_list, best_test_acc_list, benign_client_list, generalized_test_acc_list)
    ...
    graph_matrix = update_graph_matrix_neighbor(graph_matrix, nets_this_round, global_parameters, dw, fed_avg_freqs, args.alpha, args.difference_measure)
    # Graph Matrix is not normalized yet

    # the next step change 'nets_this_round' to 'cluster_models' internally
    # if one changes 'nets_this_round', then one also changes local_models, as they refer to the same object
    ...
    print('>> (Current) Round {} | Local Per: {:.5f} | Generalized: {:.5f}'.format(round, mean_personalized_acc, mean_generalized_acc))
    print('-' * 80)
...
with open('recorder_with_cosine_quantity.pkl', 'wb') as f:
